...
# ------------------------------------------------------------
# Taxonomía conocida (debe reflejar las claves de CATEGORY_LABELS y
# SUBCATEGORY_LABELS en app.py). knowledge_base.csv guarda Categoria y
# Subcategoria en un solo idioma (español); app.py traduce esos valores
# al inglés mediante esos diccionarios. Si aquí aparece una categoría o
# subcategoría que NO está en estas listas, este script no falla, pero
# imprime un aviso: hay que agregar su traducción en app.py para que se
# muestre correctamente en inglés (si no, la app usará el texto en
# español como fallback en ambos idiomas).
KNOWN_CATEGORIES = {"Cuenta", "Disputas", "Pagos", "Hold"}
KNOWN_SUBCATEGORIES = {
    "General",
    "Planes de Pago",
    "Pago No Reflejado",
    "Tiempos de Procesamiento",
    "Interés por Pago con Tarjeta",
    "Tipos de Pago Aceptados",
    "Transferencia vs. Cheque",
    ...
    "POA",
    "Pago Parcial",
    "Problemas de Báscula",
    "Error en Cotización",
    "Bloqueo de Cuenta",
    "Retención por Documentación",
}
...
def warn_unknown_taxonomy(df: pd.DataFrame) -> None:
    """
    Avisa (sin fallar) si aparecen valores de Categoria/Subcategoria que
    todavía no están traducidos en app.py (CATEGORY_LABELS /
    SUBCATEGORY_LABELS). La app los mostrará igual (usa el texto en
    español como fallback), pero para que se vean correctamente en
    inglés hay que agregarlos allá.
    """
    nuevas_categorias = sorted(
        {c for c in df["Categoria"] if c and c not in KNOWN_CATEGORIES}
    )
    nuevas_subcategorias = sorted(
        {
            s for s in df["Subcategoria"]
            if s and not is_empty_subcategoria(s) and s not in KNOWN_SUBCATEGORIES
        }
    )

    if nuevas_categorias:
        print(
            "⚠️  Aviso: nueva(s) categoría(s) sin traducción en app.py "
            f"(CATEGORY_LABELS): {nuevas_categorias}. Se mostrarán en "
            "español también en la versión en inglés hasta que se agreguen allá."
        )
    if nuevas_subcategorias:
        print(
            "⚠️  Aviso: nueva(s) subcategoría(s) sin traducción en app.py "
            f"(SUBCATEGORY_LABELS): {nuevas_subcategorias}. Se mostrarán en "
            "español también en la versión en inglés hasta que se agreguen allá."
        )


def is_empty_subcategoria(value: str) -> bool:
...
def main() -> None:
    ...
    warn_unknown_taxonomy(mapped_df)
